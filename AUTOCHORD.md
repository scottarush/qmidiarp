...
- **Chord Extensions**: Supports generating extended chords (e.g., Octaves, 5ths, 7ths, 9ths, 13ths) to add color to the harmony. The `Octave` and `5th` modes generate pure 2-note power chords, which can be combined with Root Repeats to create dynamic, driving rock sequences.
- **Root Pre/Post Count**: Allows you to dynamically repeat the root note of the chord multiple times (up to 16) before and after arpeggiating the rest of the chord degrees. For example, a Pre Count of 4 and a Post Count of 2 will play `Root x4 -> Chord Notes -> Root x2`. This injects heavy, driving root-pedal rhythms into standard sequences without needing complex custom patterns.
- **Dual Output Modes**:
...
- **Parameters**: AutoChord exposes new LV2 control ports: `State`, `Scale`, `Key Signature`, `Extensions`, `Root Pre Count`, `Root Post Count`, `Trigger Mode`, `Drum Events`, `Fill Events`, and `Gated Tail Time`. These are updated in the `updateParams()` loop.
...
- **Event Handling (`handleEvent`)**: 
  ...
  - In `AUTOCHORD_ARP` mode, a single `NOTE_ON` queries the AutoChord singleton for the chord notes. The resulting notes are sequentially pushed into QMidiArp's internal `notes` buffer via `addNote()`. If `Root Pre Count` is > 1, the root note (index 0) is artificially pushed into the buffer `N-1` additional times before the rest of the chord is evaluated. Finally, the root note is appended again exactly `Root Post Count` times at the end of the sequence, effectively forcing the sequencer to play complex root-pedal patterns.
