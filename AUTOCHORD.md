...
- **Root Repeat Count**: Allows you to dynamically repeat the root note of the chord multiple times (up to 8) before arpeggiating up the rest of the chord degrees. This injects heavy, driving root-pedal rhythms into standard sequences without needing complex custom patterns.
- **Dual Output Modes**:
...
- **Parameters**: AutoChord exposes 6 new LV2 control ports: `State`, `Scale`, `Key Signature`, `Extensions`, `Root Repeat Count`, and `Gated Tail Time`. These are updated in the `updateParams()` loop.
